...
public class DriveTrain extends Subsystem {
  ...
  public void driveTank() {
    ...
    differentialDrive.tankDrive(driveCoeff * Math.pow(joystickLeft.getY(), 1),
  ...
  /*
   * public void autoAlign(int visionCenter) { if (joystickRight.getRawButton(3))
   * { if (visionCenter - targetPixel >= 10) { while (visionCenter - targetPixel
   * >= 10) { rotate(0.25); } } if (visionCenter - targetPixel <= -10) { while
   * (visionCenter - targetPixel <= -10) { rotate(-0.25); } }
   * 
   * if(Math.abs(visionCenter - targetPixel) <= 10){ rotate(0); }
   * 
   * } }
   */
}
